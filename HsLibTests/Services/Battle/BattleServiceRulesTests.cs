...
namespace Models.Services.Battle.Tests
{
    ...
    public class BattleServiceRulesTests
    {
        ...
        public void CanBeMeleeAttackedTest()
        {
            ...
            Assert.AreEqual(false, rules.CanBeMeleeAttacked(p1Yeti, p2Yeti));
            Assert.AreEqual(false, rules.CanBeMeleeAttacked(p2Yeti, p1Yeti));

            bf[Pid.P2].Hand.Remove(p2Yeti);
            bf[Pid.P2].Field.Add(p2Yeti);
            Assert.AreEqual(false, rules.CanBeMeleeAttacked(p1Yeti, p2Yeti));
            Assert.AreEqual(false, rules.CanBeMeleeAttacked(p2Yeti, p1Yeti));
            Assert.AreEqual(true, rules.CanBeMeleeAttacked(p2Yeti, p1Hero));
            Assert.AreEqual(false, rules.CanBeMeleeAttacked(p2Yeti, p2Hero));

            bf[Pid.P1].Hand.Remove(p1Yeti);
            bf[Pid.P1].Field.Add(p1Yeti);
            Assert.AreEqual(true, rules.CanBeMeleeAttacked(p1Yeti, p2Yeti));
            Assert.AreEqual(true, rules.CanBeMeleeAttacked(p2Yeti, p1Yeti));
            Assert.AreEqual(true, rules.CanBeMeleeAttacked(p1Yeti, p2Hero));
            Assert.AreEqual(false, rules.CanBeMeleeAttacked(p1Yeti, p1Hero));
            Assert.AreEqual(true, rules.CanBeMeleeAttacked(p2Yeti, p1Hero));
            Assert.AreEqual(false, rules.CanBeMeleeAttacked(p2Yeti, p2Hero));
